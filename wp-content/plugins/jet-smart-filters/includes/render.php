...
if ( ! class_exists( 'Jet_Smart_Filters_Render' ) ) {
	...
	class Jet_Smart_Filters_Render {
		...
		public function apply_filters_from_request() {
			...
			/**
			 * Ensure we get correct current provider
			 */
			if ( ! empty( $_REQUEST['action'] ) 
				&& in_array( $_REQUEST['action'], array( 'jet_smart_filters', 'jsf' ) ) 
				&& ! empty( $_REQUEST['provider'] ) 
			) {
				$raw_provider = $_REQUEST['provider'];
			} else {
				$raw_provider = $_REQUEST['jsf'];
			}

			jet_smart_filters()->query->set_provider_from_request( $raw_provider );
