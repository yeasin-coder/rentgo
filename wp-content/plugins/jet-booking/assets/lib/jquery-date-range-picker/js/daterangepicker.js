...
(function(factory) {
    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        function handleEnd ( time ) {
            if ( checkOutDays.length ) {
                    ...
                    checkOutDay = currentCheckOutDays.reduce( ( prev, current ) => {
                        ...
                        if ( prevDiff === currentDiff ) {
                            if (
                                ...
                                || currentDisabledDays.length && currentDisabledDays.some( day => day < current )
                ...
                if (
                    ...
                    || currentDisabledDays.length && currentDisabledDays.some( day => (day > selectedDay && day < checkOutDay) || (day < selectedDay && day < checkOutDay) )
                    || maxDays.length && parseInt( maxDays ) && (moment( checkOutDate ).diff( moment( opt.start ), 'days' ) > parseInt( maxDays ))
                ) {
                    checkOutDate -= 604800000;
                }

                if ( checkOutDate < opt.start || opt.perNights && checkOutDate === opt.start ) {
                    if (
                        excludedDates.length && excludedDates.some( date => moment( date ).format( 'x' ) > opt.start )
                        || currentDisabledDays.length && currentDisabledDays.some( day => day > moment( opt.start ).day() || day < moment( opt.start ).day() )
                    ) {
        ...
        function countDays ( start, end ) {
            ...
            if (
                ! opt.perNights
                || (opt.perNights && excludedDates.length && excludedDates.some( date => moment( date ).isSame( moment( start ).format( 'YYYY-MM-DD' ) ) ))
                || (opt.perNights && disabledDays.length && disabledDays.some( day => day === moment( +start ).day() ))
            ) {
                daysCount++;
            }
